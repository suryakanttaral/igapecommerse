...
class BusinessAd {
    ...
    save=()=>{
        if (this.imagecode != "") {
            ...
            this.picpath = "businessads/" + Math.random().toString(36).substring(2, 7) + ".png";
            fs.writeFile("public/" + this.picpath, base64image, 'base64', function (err) {
                console.log("Error image saving-" + err);
            });
        }
        if(this.id == 0){
            this.query = "INSERT INTO business_ads(title, businessid, picpath, srno, link) ";
           this.query += "VALUES('"+ this.title +"', "+ this.businessid +", '" + this.picpath + "', "+ this.srno +", '" + this.link + "')"; 
        }
        else {
            this.query = "UPDATE business_ads SET title = '" + this.title + "', ";
    ...
    get=()=>{
        this.query = "SELECT * FROM business_ads WHERE id = " + this.id;
        return new Promise((resolve, reject)=>{
            this.db.query(this.query, (err, result)=>{
                this.db.close();
                if(err)
    ...
    list=()=>{
        this.query =  "SELECT * FROM business_ads WHERE businessid = " + this.businessid + " ORDER BY srno";
        return new Promise((resolve, reject)=>{
            this.db.query(this.query, (err, result)=>{
                this.db.close();
                if(err)
    ...
    delete=()=>{
        ...
        return new Promise((resolve, reject) => {
            ...
            this.query = "DELETE FROM business_ads WHERE id = " + this.id;        
              this.db.query(this.query, (err, result) => {
                if (err) {
                  this.db.close();        
                  reject(err);
